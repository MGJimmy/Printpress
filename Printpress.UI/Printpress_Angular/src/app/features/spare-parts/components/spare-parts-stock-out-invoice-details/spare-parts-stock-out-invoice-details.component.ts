...
import { SparePartSellingInvoiceListItemDto } from '../../models/spare-part-invoice-list.dto';

@Component({
  selector: 'app-spare-parts-stock-out-invoice-details',
  standalone: true,
  imports: [
    CommonModule,
    RouterLink,
  ...
  templateUrl: './spare-parts-stock-out-invoice-details.component.html',
  styleUrl: '../spare-part-invoice-list.shared.scss',
})
export class SparePartsStockOutInvoiceDetailsComponent implements OnInit {
  invoice: SparePartSellingInvoiceListItemDto | null = null;
  isLoading = false;
  isVoiding = false;
  ...
  voidInvoice(): void {
    ...
    this.dialogService.promptDialog({
      ...
      message: 'سيتم إعادة الكميات إلى المخزن وعكس حركة الخزينة المرتبطة بهذه الفاتورة. أدخل سبب الإلغاء للمتابعة.',
