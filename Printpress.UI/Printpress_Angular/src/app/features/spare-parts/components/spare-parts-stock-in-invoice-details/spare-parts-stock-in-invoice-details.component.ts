...
@Component({
  selector: 'app-spare-parts-stock-in-invoice-details',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './spare-parts-stock-in-invoice-details.component.html',
  styleUrl: '../spare-part-invoice-list.shared.scss',
})
export class SparePartsStockInInvoiceDetailsComponent implements OnInit {
  invoice: SparePartPurchaseInvoiceListItemDto | null = null;
  isLoading = false;
  isVoiding = false;
  isPaying = false;
  isReceiving = false;
  ...
  receiveGoods(): void {
    ...
    this.dialogService.confirmDialog({
      title: 'استلام للمخزن',
      message: 'سيتم إدخال كميات هذه الفاتورة إلى مخزن قطع الغيار. متابعة؟',
      ...
      this.invoiceService.receive(this.invoice!.id).pipe(
        ...
        next: () => {
          this.alertService.showSuccess('تم إدخال القطع إلى المخزن');
          this.load(this.invoice!.id);
        },
      });
    });
  ...
  voidInvoice(): void {
    ...
    const stockMsg = this.invoice.isGoodsReceived
      ? 'سيتم عكس كميات قطع الغيار وحركات الخزينة المرتبطة بهذه الفاتورة.'
      : 'سيتم عكس حركات الخزينة المرتبطة بهذه الفاتورة. القطع لم تدخل المخزن.';
